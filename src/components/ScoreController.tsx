...
export const ScoreController: React.FC<ScoreControllerProps> = ({
  ...
  if (compact) {
    return (
      <div className={`score-controller-container compact ${swapTeams ? 'swapped' : ''}`}>
        <div className="compact-teams-row">
          {teamAControl}
          {teamBControl}
        </div>
        {centerActions}
  ...
  return (
    <div className={`score-controller-container ${swapTeams ? 'swapped' : ''}`}>
      {teamAControl}
      {centerActions}
      {teamBControl}
    </div>
